...
from doppelgangers.utils.process_database import (
    create_image_pair_list,
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Structure from Motion disambiguation with Doppelgangers classification model."
    )
    parser.add_argument("config", type=str, help="The configuration file.")

    # colmap setting
    parser.add_argument(
        "--colmap_exe_command", default="colmap", type=str, help="colmap ext command"
    )
    parser.add_argument(
        "--matching_type",
    ...
    parser.add_argument(
        "--database_path", default=None, type=str, help="path to database.db"
    )
    parser.add_argument(
        "--skip_reconstruction",
        default=False,
    ...
    parser.add_argument(
        "--input_image_path", type=str, help="path to input image dataset"
    )
...
def doppelgangers_classifier(gpu, ngpus_per_node, cfg, args):
    ...
    np.save(
        os.path.join(cfg.data.output_path, "pair_probability_list.npy"),
